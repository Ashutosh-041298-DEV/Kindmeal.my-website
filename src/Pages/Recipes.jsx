...
const Recipes = () => {
  ...
  useEffect(() => {
    fetch(`https://mockserverkindmeal.herokuapp.com/recipeData?_page=${count}&_limit=12`)
      .then((res) => res.json())
  ...
  const handleSearch = (e) => {
    if (e.target.value === "") {
      fetch(`https://mockserverkindmeal.herokuapp.com/recipeData?_page=${count}&_limit=9`)
        .then((res) => res.json())
        .then((data) => setShow(data))
        .catch((er) => console.log(er));
    } else {
  ...
  return (
    <div className={style.recipe_main}>
      ...
      <div className={style.pagination}>
        ...
        <div>
        <button disabled={count===3} onClick={() => setCount(count + 1)}>Next </button>
        <button disabled={count===1} onClick={()=> setCount(count-1)}>Previous</button>
        </div>
